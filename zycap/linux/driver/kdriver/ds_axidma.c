...
struct ds_axidma_device
{
	...
	const char *dev_name;
	struct cdev c_dev;
	char *ds_axidma_addr[2];
	dma_addr_t ds_axidma_handle[2];
...
static int dmaSynchS2MM(struct ds_axidma_device *obj_dev){
  ...
  unsigned int s2mm_length = ioread32(obj_dev->virt_bus_addr + S2MM_LENGTH);
  unsigned long int timeoutval = s2mm_length * 40;
  #ifdef DEBUG
  printk(KERN_INFO "<%s> file: dmaSyncS2MM() :Current Val : %x, Current Len %d, Timeout Val %d \n", MODULE_NAME, s2mm_status, s2mm_length,timeoutval);
  #endif
    ...
    if (timeoutval > 0) {
       timeoutval--;
       #ifdef DEBUG
       printk(KERN_INFO "<%s> file: dmaSyncS2MM() :Timing Down : %d \n", MODULE_NAME, timeoutval);
       #endif
      }
    else {
      #ifdef DEBUG
      printk(KERN_INFO "<%s> file: dmaSyncS2MM() :Timed Out Status Code, Timeout Val : %x %d \n", MODULE_NAME, s2mm_status, timeoutval);
      #endif
      return 1;
      }
	}
 #ifdef DEBUG
  printk(KERN_INFO "<%s> file: dmaSyncS2MM() :Status Code, Timeout Val : %x %d \n", MODULE_NAME,  s2mm_status, timeoutval);
  #endif
	return 0;
}
...
{
	int retval;
  // printk(KERN_INFO "<%s> file: read() Length : %d \n", MODULE_NAME, len); 
  ...
  // iowrite32(0x1000, obj_dev->virt_bus_addr + MM2S_DMASR); // clear Status bit
	iowrite32(1, obj_dev->virt_bus_addr + S2MM_DMACR);
	iowrite32(obj_dev->ds_axidma_handle[0], obj_dev->virt_bus_addr + S2MM_DA);
	iowrite32(len, obj_dev->virt_bus_addr + S2MM_LENGTH);
  printk(KERN_INFO "<%s> file: read() : Controls written\n", MODULE_NAME);
  
	retval = dmaSynchS2MM(obj_dev);
 
  if (retval == 0) { 
	  memcpy(buf, obj_dev->ds_axidma_addr[0], len);
  }
  // printk(KERN_INFO "<%s> file: read() : Data Copying done\n", MODULE_NAME);
	return len;
}
			  ...
			  size_t len, loff_t * off)
{
  // printk(KERN_INFO "<%s> file: write()\n", MODULE_NAME);
	...
	obj_dev = get_elem_from_list_by_inode(f->f_inode);
	memcpy(obj_dev->ds_axidma_addr[0], buf, len);
  // iowrite32(0x1000, obj_dev->virt_bus_addr + MM2S_DMASR); // clear Status bit
	iowrite32(1, obj_dev->virt_bus_addr + MM2S_DMACR);
	iowrite32(obj_dev->ds_axidma_handle[0], obj_dev->virt_bus_addr + MM2S_SA);
	iowrite32(len, obj_dev->virt_bus_addr + MM2S_LENGTH);
  printk(KERN_INFO "<%s> file: write() : Controls written\n", MODULE_NAME);
...
static int ds_axidma_pdrv_probe(struct platform_device *pdev)
{
	...
	obj_dev->dev_name = pdev->name + 9;
        printk(KERN_INFO "Bus Address %X, Bus size %X", obj_dev->bus_addr, obj_dev->bus_size);	
	...
	/* allocate mmap area */
	obj_dev->ds_axidma_addr[0] =
	    dma_zalloc_coherent(NULL, DMA_LENGTH, &(obj_dev->ds_axidma_handle[0]), GFP_KERNEL);
...
static int ds_axidma_pdrv_remove(struct platform_device *pdev)
{
	...
	list_for_each_safe( pos, q, &full_dev_list ) {
    	...
    	if (!my_strcmp(obj_dev->dev_name, pdev->name + 9))
    	{
    		...
    		unregister_chrdev_region(obj_dev->dev_num, 1);
    		/* free mmap area */
			if (obj_dev->ds_axidma_addr[0]) {
				dma_free_coherent(NULL, DMA_LENGTH, obj_dev->ds_axidma_addr[0], obj_dev->ds_axidma_handle[0]);
			}
    		kfree(obj_dev);
    		break;
    	}
